...
\begin{frame}
	\mframe{\Arch}
	\begin{alertblock}{Assembleur}
		\begin{itemize}
			\item<1-> Le langage machine est le seul compréhensible par un processeur, il se compose uniquement de suites de 0 et de 1. Ecrire un programme avec ce langage est extrêmement compliqué pour un être humain.
			\item<2-> Le langage assembleur traduit en code mnémotechnique les instructions machine les rendant plus accessibles à un être humain.\\
			\onslide<3->{\textcolor{OliveGreen}{Par exemple {\tt ADD R2,R1,R0} signifie additionner les valeurs des registres R0 et R1 et stocker le résultat dans R2 se traduit en une suite de 0 et de 1.}}
			\item<3-> Chaque assembleur est  spécifique à un processeur (ou à une famille de processeur).
			\item<4-> Dans les langages de haut niveau (comme Python), c'est l'interpréteur (ou le compilateur) qui se charge de la traduction des instructions en langage machine. Ces langages sont donc indépendants des processeurs utilisés.
		\end{itemize}
	\end{alertblock}
\end{frame}
