...
\begin{Exercice}[\textit{Base de données}]
 ...
 SET, VALUES, COUNT, OR, AND}.
  ...
  \begin{itemize}
    \item \mintinline{SQL}{Articles (#idArticle, titre, #auteur, dateParution)}
    \item \mintinline{SQL}{Auteurs (#idAuteur, nom, prenom)}
    \item \mintinline{SQL}{Themes (#idTheme, theme)}
    \item \mintinline{SQL}{Traitements (#idTraitement, #article, #theme)}
  \end{itemize}
...
def f(arbre: list, i: int) -> int:
  ...
  \begin{Question}
    Compléter sur cet énoncé le code de la fonction suivante indiquant si un arbre est équilibré ou non. On rappelle que par définition un arbre vide est équilibré. \mintinlinepy{arbre} est la liste \python\ contenant un arbre et \mintinlinepy{i} l'indice d'un nœud, racine du du sous-arbre étudié.
